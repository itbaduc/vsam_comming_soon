...
window.Modernizr = (function (Z, Y, X) {
  ...
  var W = '2.8.3',
    ...
    H = {}.hasOwnProperty,
    G;
  !D(H, 'undefined') && !D(H.call, 'undefined')
    ? (G = function (d, c) {
        return H.call(d, c);
      })
    : (G = function (d, c) {
    ...
    (N.cssgradients = function () {
      ...
      return (
        F((e + '-webkit- '.split(' ').join(d + e) + O.join(f + e)).slice(0, -e.length)),
        C(R.backgroundImage, 'gradient')
      );
    });
  for (var A in N) {
    ...
    (V.addTest = function (e, c) {
      if (typeof e == 'object') {
        ...
        (c = typeof c == 'function' ? c() : c),
          typeof enableClasses != 'undefined' && enableClasses && (U.className += ' ' + (c ? '' : 'no-') + e),
          (V[e] = c);
      }
      return V;
    }),
...
(function (e, c, a, g) {
  var d = 'gradientify',
    f = { angle: '0deg', fps: 60, gradients: {}, transition_time: 8 };
  function b(i, h) {
    this.element = i;
  ...
  e.extend(b.prototype, {
      ...
      for (var j in this.rgb_values) {
        if (this.rgb_values.hasOwnProperty(j)) {
          for (var h = 0; h < 3; h++) {
            ...
            this.rgb_steps[j][h] = this.calculateStepSize(
              this.settings.gradients[this.nextIndex][j][h],
              this.rgb_values[j][h]
            );
          }
        }
    ...
    updateGradient: function () {
      ...
      var l =
        'rgb(' +
        (this.rgb_values.start[0] | 0) +
        ',' +
        (this.rgb_values.start[1] | 0) +
        ',' +
        (this.rgb_values.start[2] | 0) +
        ')';
      var h =
        'rgb(' +
        (this.rgb_values.stop[0] | 0) +
        ',' +
        (this.rgb_values.stop[1] | 0) +
        ',' +
      ...
      if (l != this.color1 || h != this.color2) {
        ...
        e(this.element).css(
          'background-image',
          '-webkit-gradient(linear, left bottom, right top, from(' + this.color1 + '), to(' + this.color2 + '))'
        );
        for (k = 0; k < 4; k++) {
          e(this.element).css(
            'background-image',
            this.prefixes[k] + 'linear-gradient(' + this.settings.angle + ', ' + this.color1 + ', ' + this.color2 + ')'
          );
        }
      }
      this.steps_count++;
